...
{
	{
		...
		//remove old ready queues w el quantums
		sched_delete_ready_queues();
		//set num of ready queues
		num_of_ready_queues = numOfPriorities;
#if USE_KHEAP
		//allocate ready queues
		ProcessQueues.env_ready_queues = kmalloc(num_of_ready_queues* sizeof(struct Env_Queue));
		memset(ProcessQueues.env_ready_queues, 0, num_of_ready_queues* sizeof(struct Env_Queue));
		//allocate quantum array
		quantums = kmalloc(num_of_ready_queues * sizeof(uint8));
#endif
		//initialize all ready queues w nafs el quantum l kolo
		...
		// set cpu quantum for initial priority
		kclock_set_quantum(quantums[0]);
...
struct Env* fos_scheduler_PRIRR()
{
	...
	//requeue current ready l a5er el queue
	if(cur!= NULL && cur->env_status==ENV_READY){
		enqueue(&ProcessQueues.env_ready_queues[cur->priority], cur);

	}
	//pick awel non empty highest priority queue
	for(int p =0; p<num_of_ready_queues; p++){
		if(!LIST_EMPTY(&ProcessQueues.env_ready_queues[p])){
...
{
	if (isSchedMethodPRIRR() && starvation_threshold >0)
	{
        ...
        //scan all queues ma3ada priority  0
        for(int p = num_of_ready_queues-1; p>=1; p--){
        	struct Env* e = LIST_FIRST(&ProcessQueues.env_ready_queues[p]);
        	struct Env* next;
        	while(e!=NULL){
        		next = LIST_NEXT(e);
        		...
        		e->nClocks++;
        		//check el starvation
        		if(e->nClocks >= starvation_threshold){
        			LIST_REMOVE(&ProcessQueues.env_ready_queues[p], e);
        			//promote one level
        			e->priority = p-1;
        			e->nClocks = 0;
        			LIST_INSERT_HEAD(&ProcessQueues.env_ready_queues[p-1], e);
        ...
        //increment el clock bta3t priority 0
        struct Env* e0 = LIST_FIRST(&ProcessQueues.env_ready_queues[0]);
        while (e0 != 0){
        	e0->nClocks++;
        	e0 = LIST_NEXT(e0);
        }
...
void update_WS_time_stamps()
{
    ...
    //loop 3la el ws(linked list)
    LIST_FOREACH(ptr, &(e->page_WS_list)) {

        uint32 va = ptr->virtual_address;

        //bget el perms llva mn el page dir
        uint32 perms = pt_get_page_permissions(e->env_page_directory, va);

        //aging algo of page replace
        //bright shift el timestamp b"1"
        ptr->time_stamp = ptr->time_stamp >> 1;

        if(perms & PERM_USED) {
        	//if page accessed --> set el most significant bit b1(31)
            //da eli by5liha ru
            ptr->time_stamp = ptr->time_stamp | 0x80000000;

            //clear el flag fl table entry
            //reset el accessed bit
