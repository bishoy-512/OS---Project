...
{
	// it checks the lock and change it in case it was called outside the run all and run env and all that
	bool already_locked = holding_kspinlock(&ProcessQueues.qlock);
	if(!already_locked) acquire_kspinlock(&ProcessQueues.qlock);
	...
	if(!already_locked) release_kspinlock(&ProcessQueues.qlock);
}
...
void sched_remove_ready(struct Env* env)
{
	...
	{
		if(env->priority >= 0 && env-> priority < num_of_ready_queues)
		{
				LIST_REMOVE(&(ProcessQueues.env_ready_queues[env->priority]), env);
...
void env_set_priority(int envID, int priority)
{
	...
	struct Env *e = NULL;
	//extract the environment index from el id
	int idx = ENVX(envID);
	e = &(envs[idx]);
	struct Env *tmp;
	LIST_FOREACH(tmp, &env_free_list){
		if(tmp == e) {
			cprintf("env_set_priority: env %u not found! \n", envID);
			return;
		}
    }
	//setting the new priority
	e->priority = priority;
	//reorder el position in the ready queue
