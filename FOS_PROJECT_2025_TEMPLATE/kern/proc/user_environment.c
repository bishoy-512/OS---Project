...
struct Env_list env_free_list;	// Free Environment list
...
void *create_user_kern_stack(uint32 *ptr_user_page_directory)
{
	...
	/*first, try to allocate the user stack using kmalloc */
	void *st_ptr = kmalloc(KERNEL_STACK_SIZE);
	/*check if it's allocated or not*/
	if (st_ptr == NULL)
		panic("No Space Enough In Kernel");
	pt_set_page_permissions(ptr_user_page_directory, (uint32)st_ptr, 0, PERM_PRESENT);
	return st_ptr;
}
