...
inline struct PageRefElement*  env_page_pageRef_list_create_element(struct Env* e, uint32 virtual_address)
{
	assert(virtual_address >= 0 && virtual_address < USER_TOP);
		struct PageRefElement *wse = kmalloc(sizeof(struct WorkingSetElement)) ;
		if (wse == NULL)
		{
			panic("can't create a new WS element");
		}
