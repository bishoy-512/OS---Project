...
//		return;
	...
	// get the page table dir , start , end addresses
	...
	// loop for mark the needed pages
	for (uint32 addr = start; addr < end; addr += PAGE_SIZE) {
		// if page table is not exist => create one
		uint32 *pt;
		int ret = get_page_table(pd, addr, &pt);
		if (ret == TABLE_NOT_EXIST)
			pt = create_page_table(pd, addr);
		pt_set_page_permissions(pd, addr, PERM_UHPAGE | PERM_USER | PERM_WRITEABLE, 0);
	}
...
//		return;
	...
	// loop for unmark the given pages and remove it from working set and page
	for (uint32 i = start; i < end; i += PAGE_SIZE) {
		pt_set_page_permissions(pd, i, 0, PERM_UHPAGE | PERM_PRESENT | PERM_USER );
		pf_remove_env_page((struct Env*)e , i);
		env_page_ws_invalidate((struct Env*)e , i);
	}
