...
inline int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
	...
	/* first => check if page table exist */
    uint32* pt;
    int is_table_exist = get_page_table(directory, virtual_address, &pt);
    /* if Not exist => return -1 */
    if (is_table_exist == TABLE_NOT_EXIST)
       return -1;
    /* if page table exist => return the last 12 bits */
    uint32 obj = pt[PTX(virtual_address)];
    return obj & 0xFFF;
...
inline void pt_clear_page_table_entry(uint32* directory, uint32 virtual_address)
{
    ...
    /* if Not exist => Panic() */
    if (is_table_exist == TABLE_NOT_EXIST)
        panic("Page Table Is Not Exist :)\n");
    /* if page table exist => ( Clear The Entry & invalidate cache ) */
	pt[PTX(virtual_address)] = 0;
	tlb_invalidate(directory, (void*)virtual_address);
}
